...
<script lang="ts">
	...
	let {
		items = [],
		activePath = '',
		title: _title = 'Documentation',
		showSearch: _showSearch = true
	}: DocsSidebarProps = $props();
	...
	let filteredItems = $derived(searchQuery ? filterItems(items, searchQuery.toLowerCase()) : items);
			...
			{#each filteredItems as item, index}
								...
								{#each item.children as child}
									<li>
										{#if child.children && child.children.length > 0}
											<!-- Nested section -->
											<div
												class="text-xs font-medium text-[var(--color-muted)] uppercase tracking-wider px-3 py-2 mt-2"
											>
												{child.title}
											</div>
											<ul class="space-y-1">
                ...
                {isActive(item.url)
					...
					{:else}
						...
						<div
							class="px-3 py-2 text-xs font-medium text-[var(--color-muted)] uppercase tracking-wider"
						>
							{item.title}
						</div>
					{/if}
