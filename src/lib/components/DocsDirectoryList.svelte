...
<script lang="ts">
	...
	let {
		title = '',
		description = 'Guides, resources, and references to help you build with Statue.',
		content = [],
		subDirectories = [],
		primaryButtonText = '',
		primaryButtonUrl = '',
	...
	let resolvedPrimaryText = $derived(
		primaryButtonText || featuredContent[0]?.metadata?.title || 'Get Started'
	);
	let resolvedPrimaryUrl = $derived(primaryButtonUrl || featuredContent[0]?.url || '/docs');
	let resolvedSecondaryText = $derived(
		secondaryButtonText || featuredContent[1]?.metadata?.title || ''
	);
...
<article class="docs-directory relative">
	...
	<div
		class="absolute top-32 -right-20 w-[500px] h-[500px] bg-[var(--color-primary)] opacity-[0.08] blur-[150px] rounded-full pointer-events-none"
	></div>
					...
					>
						...
						<div class="flex items-start gap-4">
							...
							<div class="flex-1 min-w-0">
								<h3
									class="font-semibold text-[var(--color-foreground)] group-hover:text-[var(--color-primary)] transition-colors mb-1"
								>
							...
							<svg
								class="flex-shrink-0 w-5 h-5 text-[var(--color-muted)] group-hover:text-[var(--color-primary)] group-hover:translate-x-0.5 transition-all mt-1"
								fill="none"
								viewBox="0 0 24 24"
								stroke="currentColor"
							>
	...
	{#if (!content || content.length === 0) && (!subDirectories || subDirectories.length === 0)}
		<div class="text-center py-12">
			<svg
				class="mx-auto w-12 h-12 text-[var(--color-muted)]"
				fill="none"
				viewBox="0 0 24 24"
				stroke="currentColor"
			>
