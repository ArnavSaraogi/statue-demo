		...
		>
			...
			{#each results as result, index}
				<a
					...
					href={result.url}
					class="search-result block p-3 border-b border-[var(--color-border)] last:border-b-0 transition-colors {selectedIndex ===
					index
				...
				>
					...
					<div class="flex items-start justify-between gap-2 mb-1">
						<h3
							class="text-sm font-medium line-clamp-1 {selectedIndex === index
								? ''
						...
						{#if showCategories}
							<span
								class="flex-shrink-0 px-2 py-0.5 text-xs rounded-full {selectedIndex === index
									? 'bg-[var(--color-on-primary)] text-[var(--color-primary)]'
									: 'bg-[var(--color-border)] text-[var(--color-foreground)]'} border border-[var(--color-border)]"
							>
					...
					{#if showExcerpts && result.excerpt}
						<p
							class="text-xs line-clamp-2 mb-1 {selectedIndex === index
								? 'opacity-90'
					...
					<!-- Meta info -->
					<div
						class="flex items-center gap-2 text-xs {selectedIndex === index
							? 'opacity-80'
							: 'text-[var(--color-muted)]'}"
					>
