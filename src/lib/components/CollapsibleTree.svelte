...
<script lang="ts">
	...
	let { items = [], title = 'Collapsible Tree' }: CollapsibleTreeProps = $props();
...
<div
	class="rounded-2xl border border-[color:var(--color-border)] bg-[color:var(--color-card)]/50 backdrop-blur-sm p-6"
>
	...
	<div class="space-y-0.5">
		{#each flatItems as item}
			...
			>
				...
				<!-- Icon -->
				<span
					class="flex h-6 w-6 items-center justify-center rounded-md bg-[color:var(--color-primary)]/10 flex-shrink-0"
				>
					<svg
						class="w-3.5 h-3.5 text-[color:var(--color-primary)]"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
					>
				...
				{#if item.badge}
					<span
						class="px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 {getBadgeClass(
							item.badge
						)}"
					>
						{item.badge}
