<script lang="ts">
	...
	function initThree() {
		...
		const coreMaterial = new THREE.MeshBasicMaterial({
			color: getThemeColor('--color-primary', '#58a6ff')
		});
...
<div class="hud">
	...
	<div class="hud-center">
		{#each timelineItems as item, i}
			<div
				class="experience-card {activeIndex === i ? 'active' : ''}"
				style="--acc: {item.colorVar ? `var(${item.colorVar}, ${item.color})` : item.color}"
			>
				<div
					class="exp-icon"
					style="background: {item.colorVar ? `var(${item.colorVar}, ${item.color})` : item.color}"
				>
					{#if item.type === 'work'}
						<svg
							width="20"
