<script lang="ts">
	...
	function handleWindowMouseMove(event: MouseEvent) {
		if (isDraggingIcon && draggedIconId) {
			...
			const dist = Math.sqrt(
				Math.pow(event.clientX - startClickPos.x, 2) + Math.pow(event.clientY - startClickPos.y, 2)
			);
	...
	function savePositions() {
		if (typeof localStorage !== 'undefined') {
			const positions = desktopItems.reduce(
				(acc, item) => {
...
<div class="desktop">
	...
	{#each desktopItems as item}
		...
		<div
			class="desktop-icon"
			style="top: {item.defaultPosition.y}px; left: {item.defaultPosition
				.x}px; cursor: {isDraggingIcon && draggedIconId === item.id
				? 'grabbing'
				: 'pointer'}; transform: {isDraggingIcon && draggedIconId === item.id
				? 'scale(1.1)'
	...
	<div class="dock-container">
		<div class="dock">
			{#each dockItems as item}
				<button class="dock-item" on:click={() => handleDockClick(item)} title={item.name}>
...
<style>
	...
	.desktop {
		width: 100vw;
		height: 100vh;
		background: url('https://images.unsplash.com/photo-1477346611705-65d1883cee1e?auto=format&fit=crop&w=2000&q=80')
